...
def decide_cut(
    tokens: List[str],
    chance_to_truncate: float,
    punctuation_cut_chance: float
) -> Optional[int]:
    """
    Decide if and where to truncate the last sentence, returning a token index or None.

    Args:
        tokens (List[str]): List of whitespace-split tokens from the last sentence.
        chance_to_truncate (float): Probability (0..1) that truncation happens at all.
        punctuation_cut_chance (float): Probability (0..1) that, if truncation happens,
            we cut at a punctuation-based token ('.', '!', '?') rather than a random token boundary.

    Returns:
        Optional[int]: The token index (exclusive) at which to cut 
                       (i.e. keep tokens[:cut_index]) or None if no truncation.
    """
    import random

    # Decide if truncation occurs
    if random.random() >= chance_to_truncate:
        return None  # No truncation

    # If we do truncate, pick punctuation vs random
    if random.random() < punctuation_cut_chance:
        # Find tokens that end in punctuation
        punct_indices = [i for i, tok in enumerate(tokens) if tok.endswith(('.', '!', '?'))]
        if punct_indices:
            chosen_idx = random.choice(punct_indices)
            return chosen_idx + 1  # keep up to and including that token
        else:
            # Fallback: random token boundary
            chosen_idx = random.randint(1, len(tokens))
            return chosen_idx
    else:
        # Straight random cut
        chosen_idx = random.randint(1, len(tokens))
...
def apply_cut(text: str, cut_index: Optional[int]) -> str:
    """
    Apply the given cut_index to the last sentence of the text.
    If cut_index is None or if the text has fewer than 3 total sentences,
    we return the text as-is (i.e., no truncation).

    Args:
        text (str): Original text
        cut_index (Optional[int]): The token index to truncate after. None means no truncation.

    Returns:
        str: Possibly truncated text (applied to the last sentence only).
    """
    import re

    # Regex: capture sentences with trailing punctuation
    sentence_pattern = r'([^.!?]+[.!?])'
    ...
    # NEW LOGIC: If there are fewer than 3 total sentences, do not truncate
    if len(sentences) < 3:
        return text.strip()

    # If we have at least 3 sentences, we may truncate if cut_index is not None
    if not sentences:
        return text.strip()

    last_sentence = sentences[-1]

    # If cut_index is None, skip truncation
    if cut_index is None:
        return text.strip()
    
    # Otherwise, split the last sentence into tokens and see if we can cut
    tokens = last_sentence.split()
    if 0 < cut_index < len(tokens):
        # Keep up to cut_index tokens
        truncated_tokens = tokens[:cut_index]
        truncated_last = " ".join(truncated_tokens)
        # Rebuild
        if len(sentences) > 1:
            new_text = " ".join(sentences[:-1]) + " " + truncated_last
        else:
            new_text = truncated_last
        return new_text.strip()

    # If cut_index is beyond the last token or <= 0, effectively no truncation
    return text.strip()
...
class ConceptExampleGenerator:
    ...
    def generate_examples(self, 
        ...
        if example_length == "short":
            ...
            length_guide = "For each example, write four to five sentences. Ensure that examples are at least four full sentences long. Do not generate any examples that are less than four sentences long. Make sure that the concept is present in the whole text."
        ...
        for attempt in range(max_retries):
            try:
                ...
                for i, ex in enumerate(examples):
                    if not isinstance(ex, dict) or "positive" not in ex or "negative" not in ex:
                        logging.warning(f"Skipping example {i+1} due to invalid format")
                        continue

                    # Parse last sentence from the positivie example
                    sentence_pattern = r'([^.!?]+[.!?])'
                    pos_sentences = re.findall(sentence_pattern, ex["positive"])
                    pos_sentences = [s for s in pos_sentences if s.strip()]

                    if pos_sentences:
                        pos_last = pos_sentences[-1]
                        pos_tokens = pos_last.split()

                        # Decide a single cut for both
                        cut_index = decide_cut(
                            tokens=pos_tokens,
                            chance_to_truncate=0.8,     # Example
                            punctuation_cut_chance=0.1  # Example
                        )
                    else:
                        cut_index = None

                    # Apply the same cut to both strings
                    truncated_pos = apply_cut(ex["positive"], cut_index)
                    truncated_neg = apply_cut(ex["negative"], cut_index)

                    validated_examples.append({
                        "positive": truncated_pos,
                        "negative": truncated_neg,
                    })
