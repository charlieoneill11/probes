...
# Helper function for truncating text later on - post-processing of example text
def random_truncate(
    text: str,
    chance_to_truncate: float = 0.5,
    punctuation_cut_chance: float = 0.3) -> str:
    """
    Retain all sentences from the input text, but with a user-defined chance,
    truncate the last sentence either at a token ending in punctuation or at 
    a random token boundary (never mid-word). 
    
    Args:
        text (str): The original text.
        chance_to_truncate (float): Probability (0.0 to 1.0) that the last sentence 
            will be truncated at all. Defaults to 0.5.
        punctuation_cut_chance (float): Probability (0.0 to 1.0) that, if truncation 
            happens, we cut at a punctuation-based token boundary ('.', '!', '?'). 
            The remainder of the time, we cut at a random token boundary. Defaults to 0.3.
    
    Returns:
        str: The processed example.
    """
    # Identify sentences and keeping trailing punctuation
    sentence_pattern = r'([^.!?]+[.!?])'
    ...
    # If no complete sentences are found, return the original text
    if not sentences:
        return text.strip()
    
    # Isolate the last sentence for potential truncation
    last_sentence = sentences[-1]
    
    # Randomisation of truncation
    if random.random() < chance_to_truncate:
        # Split last sentence into tokens
        tokens = last_sentence.split()
        
        # Decide whether to truncate at punctuation or a random token boundary
        if random.random() < punctuation_cut_chance:
            # Truncate after a token that ends with . ! or ?
            punct_indices = [
                i for i, tok in enumerate(tokens) 
                if tok.endswith(('.', '!', '?'))
            ]
            if punct_indices:
                # Pick one punctuation token at random
                chosen_idx = random.choice(punct_indices)
                # Keep tokens through that token (inclusive)
                truncated_tokens = tokens[:chosen_idx + 1]
            else:
                # If no token ends in punctuation, fall back to random token boundary
                chosen_idx = random.randint(1, len(tokens))
                truncated_tokens = tokens[:chosen_idx]
        else:
            # Truncate at a random token boundary (but not mid-word)
            chosen_idx = random.randint(1, len(tokens))  # up to len(tokens) inclusive
            truncated_tokens = tokens[:chosen_idx]
        
        # Reconstruct the last sentence
        last_sentence = " ".join(truncated_tokens)
    
    # Rebuild all sentences with the (possibly truncated) last sentence
    if len(sentences) > 1:
        # Combine everything except the old last sentence, then add new last sentence
        truncated_text = " ".join(sentences[:-1]) + " " + last_sentence
    else:
        truncated_text = last_sentence
        ...
        for attempt in range(max_retries):
            try:
                ...
                for i, example in enumerate(examples):
                    ...
                    # Apply random truncation to each positive/negative
                    truncated_pos = random_truncate(example["positive"], chance_to_truncate=0.8, punctuation_cut_chance=0.1)
                    truncated_neg = random_truncate(example["negative"], chance_to_truncate=0.8, punctuation_cut_chance=0.1)
